...
<x-base-section>
  ...
  <div class="w-full max-w-xs">
    <form action="{{route('update.user',['id'=>$id])}}" method="post" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
       @csrf
       @method('patch')
  <h1 class="mb-4 font-bold text-pink-500">User profile</h1>
  @if (session('status'))
  ...
  @endif
    <div class="mb-4">
      ...
      border-red-500
      @enderror shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" name="name" id="name" type="text" value="{{$user->name}}" placeholder="{{$user->name}}">
      @error('name')
    ...
    <div class="mb-4">
      ...
      border-red-500
      @enderror shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" name="email" id="email" type="text" value="{{$user->email}}" placeholder="{{$user->email}}">
      @error('email')
      <p class="text-red-500 text-xs italic">{{$message}}</p>
      @enderror
    </div> 
